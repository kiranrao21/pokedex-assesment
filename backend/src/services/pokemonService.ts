...
class PokemonService {
  async fetchPokemonList(limit: number = 20, offset: number = 0, search?: string): Promise<APIResponse<Pokemon[]>> {
    try {
      // Call to PokeAPI
      ...
      const pokemons: Pokemon[] = await Promise.all(
        data.results.map(async (result: { name: string; url: string }) => {
          ...
          if (!pokemonResponse.ok) {
            throw new Error(`Failed to fetch pokemon details for ${result.name}`);
          }
          const pokemonData = await pokemonResponse.json() as PokeAPIDetailResponse;
